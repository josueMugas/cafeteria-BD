...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-06-2026 a las 19:19:19
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ActualizarProducto` (IN `p_id_prod` INT, IN `p_nuevo_nombre` VARCHAR(50), IN `p_nuevo_precio` DECIMAL(10,2), IN `p_nueva_cantidad` INT)   BEGIN
    UPDATE productos 
...
DROP PROCEDURE IF EXISTS `ConsultarHistorialCliente`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ConsultarHistorialCliente` (IN `p_id_user` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarCliente` (IN `c_nom` VARCHAR(50), IN `c_mail` VARCHAR(50), IN `c_contra` VARCHAR(255), IN `c_dom` VARCHAR(50))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Registrar_Compra` (IN `p_id_usuario` INT, IN `p_id_producto` INT, IN `p_cantidad` INT)   BEGIN
...
CREATE TABLE `clientes` (
  ...
  `fondos` decimal(20,2) DEFAULT 0.00
...
(44, 'Mega Queque Final Dorado 24QLT (Entero)', 1, 99999999.00, 'dios.png.png', 'Pasteleria');
...
(11, 'Admin', 'josue.mugaset32@gmail.com', '$2y$10$KidZWrb.pyHQcki4wxH6BuGY/V559Lw0EjLVply0QrcYEIerq.MtS', 'quesada 2286', 'Admin'),
(13, 'josue', 'josue1.mugaset32@gmail.com', '$2y$10$pWSd4fYQIJKHx215pyu8iOCLhD4RwLatYRsk7wHVaebuSCdIUgkI6', 'quesada 2286', 'Cliente'),
...
DROP VIEW IF EXISTS `resumen_pedidos`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `resumen_pedidos`  AS SELECT `p`.`id_pedido` AS `id_pedido`, `u`.`nombre` AS `nombre_cliente`, `u`.`mail` AS `email_cliente`, `prod`.`nombre` AS `producto`, `prod`.`categoria` AS `categoria_producto`, `p`.`cantidad_producto` AS `cantidad`, `prod`.`precio` AS `precio_uni`, `p`.`cantidad_producto`* `prod`.`precio` AS `total_pedido`, `p`.`fecha` AS `fecha_pedido` FROM (((`pedidos` `p` join `clientes` `c` on(`p`.`id_usuario` = `c`.`id_cliente`)) join `usuarios` `u` on(`c`.`id_usuario` = `u`.`id_usuario`)) join `productos` `prod` on(`p`.`id_producto` = `prod`.`id_producto`)) ;
